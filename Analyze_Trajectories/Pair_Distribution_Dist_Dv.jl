function Combined_Histograms_d_v(N, cam, path, Δd, d_max, fps, f_min)
    ...
    for x in cam
        ...
        for y in N
            ...
            Add_Histograms_d_v!(data, Δd, d_max, d_inter, d_indep, f_min, dt)
...
function Add_Histograms_d_v!(data, Δd, d_max, d_inter, d_indep, f_min, dt)
    ...
    for i in 1:length(frames)
        ...
        for j in i+1:length(frames)
            ...
            if x == false
                ...
                Add_Ind_Data_d_v!(data, i, j, d_indep, frames, rows, Δd, dt, d_max)

            elseif x[2]-x[1] >= f_min

                #if this they share at least f_min frames: it is used to estimate the "interacting" distribution
                Add_Int_Data_d_v!(data, i, j, d_inter, frames, rows, x, Δd, dt, d_max)
...
function Add_Int_Data_d_v!(data::CSV.File, i, j, hist_inter, frames, rows, x, step, dt, d_max)
    ...
    for m in x[1]:x[2]-d_pos
        ...
        dv = v_1D(data, l, l+d_pos, (d_pos-1)*dt)-v_1D(data, k, k+d_pos, (d_pos-1)*dt)
...
function Add_Ind_Data_d_v!(data::CSV.File, i, j, hist_inter, frames, rows, Δd, dt, d_max)
    ...
    for k in rows[i]:10:rows[i]+(frames[i][2]-frames[i][1]-d_pos)
        ...
        for l in rows[j]:10:rows[j]+(frames[j][2]-frames[j][1]-d_pos)
            ...
            #dv = v_1D(data, l, l+d_pos, (d_pos-1)*dt)-v_1D(data, k, k+d_pos, (d_pos-1)*dt)
                ...
                hist_inter[Int(round((d(data, k, l))/Δd))+1, 1]+=1

            #elseif dv <= 20

                #hist_inter[Int(round((d(data, k, l))/Δd))+1, 2]+=1

            #elseif dv >= 20

                #hist_inter[Int(round((d(data, k, l))/Δd))+1, 3]+=1
